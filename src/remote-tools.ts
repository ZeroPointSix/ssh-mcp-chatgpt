...
export function buildManagedRemoteCancelCommand(jobId: string): string {
  ...
  const inner = [
    ...
    "[ \"$pid\" -gt 1 ] || exit 4",
    "group_has_live_processes() { if [ ! -d /proc ]; then kill -0 -- \"-$pid\" 2>/dev/null; return; fi; for stat_path in /proc/[0-9]*/stat; do [ -r \"$stat_path\" ] || continue; IFS= read -r stat < \"$stat_path\" || continue; rest=\"${stat##*) }\"; set -- $rest; [ \"${3:-}\" = \"$pid\" ] || continue; [ \"${1:-}\" = Z ] || return 0; done; return 1; }",
    "kill -0 -- \"-$pid\" 2>/dev/null || finish_cancel",
    "kill -TERM -- \"-$pid\" 2>/dev/null || true",
    "attempt=0",
    "while group_has_live_processes && [ \"$attempt\" -lt 10 ]; do attempt=$((attempt + 1)); sleep 0.1; done",
    "group_has_live_processes || finish_cancel",
    "kill -KILL -- \"-$pid\" 2>/dev/null || true",
    "attempt=0",
    "while group_has_live_processes && [ \"$attempt\" -lt 10 ]; do attempt=$((attempt + 1)); sleep 0.1; done",
    "group_has_live_processes && exit 5",
    "finish_cancel",
  ].join("; ");
  return "bash -c " + quoteShell(inner);
}
